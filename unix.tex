...
Основной работой операционной системы является распределение ресурсов 
компьютера среди нескольких программ, а также предоставление более полезного 
фукционала для непосредственной работы с железом, которое оно само по себе может предоставить.
Операционная система усправляет и абстрагирует работу с низкоуровневым железом 
таким образом, что текстовому процессору не нужно нагружать себя мыслями, 
какой тип дискового хранилища используется в данный момент.
Операционная система распределяет ресурсы железа между несколькими программами так, 
чтобы они выполнялись (или, казалось бы, что выполняются) одновременно.
И наконец, операционные системы предоставляют контроллируемые пути для взаимодействия 
программ друг с другом так, чтобы они могли делиться данными или работать вместе.

Операционная система предоставляет сервисы для пользовательских программ через интерфейс.
\index{interface design}
Проектирование хорошего интерфейса иногда представляется трудной задачей. С одной стороны, мы хотим чтобы интерфейс был простой и небольшой, потому что это позволяет реализовать его с меньшим количеством проблем. С другой стороны, у нас может возникнуть соблазн предложить приложениям множество сложных функций. Хитрость в том, чтобы суметь разрешить эти противоречия таким образом, чтобы спроектировать интерфейсы, которые основываются на небольшом количестве механизмов, но которые могут быть совмещены для обеспечения наибольшего функционала.

Эта книга использует одну операционную систему как конкретный пример иллюстрации концептов операционных систем. Эта операционная система, под названием xv6, предоставляет простые интерфейсы, впервые представленные Кеном Томпсоном и Деннисом Ритчи, в операционной системе Unix~\cite{unix}, а также подражает внутреннему дизайну Unix. Unix предоставляет небольшой интерфейс, механизмы которого собраны так, чтобы предложить поразительный градус общности. Этот интерфейс настолько успешен, что современные операционные системы – BSD, Linux, macOS, Solaris и, в меньшей степени Microsoft Windows, имеют на своем борту эти Unix-подобные интерфейсы. Понимание работы xv6 будет являться хорошей стартовой точкой для понимания работы этих и других операционных систем.


Как показывает
Рисунок~\ref{fig:os}, xv6 представляет собой традионную форму ядра ОС \indextext{kernel}
